import { REDIS_PASSWORD } from '@fever/environment'
import { HealthStatus } from '@fever/types'
import { Redis } from 'ioredis'

import { ICacheService } from '../../domain/repositories/ICacheService.js'
import { CacheConfig, CacheHealthStatus } from '../../domain/types/cache.js'
...
export class RedisService implements ICacheService {
   ...
   */
  constructor(config?: CacheConfig & { password?: string }) {
    this.config = RedisConfigService.getInstance()

    const redisConfig = this.config.getConfig()

    // For test environments, prefer environment variables over cached config
    // This allows tests to set Redis connection details dynamically
    const host = config?.host || process.env.REDIS_HOST || redisConfig.host
    const port =
      config?.port ||
      parseInt(process.env.REDIS_PORT || String(redisConfig.port), 10)
    const password =
      config?.password ||
  ...
  public async checkHealth(): Promise<CacheHealthStatus> {
    ...
    try {
      ...
      return {
        status:
          this.errorCount > 0 ? HealthStatus.DEGRADED : HealthStatus.HEALTHY,
        details: {
          connection: true,
          latency,
  ...
  public async get<T>(key: string): Promise<T | null> {
    ...
    try {
      ...
      try {
        ...
        logger.error('Error parsing cached value', { key }, parseError)
      ...
      logger.error('Error retrieving cached value', { key }, error)
  ...
  public async set(
    ...
    try {
      ...
      logger.error('Error setting cached value', { key }, error)
  ...
  public async setNX(
    ...
    try {
      ...
      logger.error('Error setting cached value (NX)', { key }, error)
  ...
  public async updateTTL(key: string, ttl: number): Promise<boolean> {
    try {
      ...
      logger.error('Error updating TTL', { key }, error)
